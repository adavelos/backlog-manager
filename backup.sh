...
DB_PATH="$HOME/.backlog/data/backlog.db"
KEEP_COUNT=7
...
BACKUP_FILE="$BACKUP_DIR/backlog_$TIMESTAMP.db"
...
BACKUP_COUNT=$(ls -1 "$BACKUP_DIR"/backlog_*.db 2>/dev/null | wc -l)
if [ "$BACKUP_COUNT" -gt "$KEEP_COUNT" ]; then
  EXCESS=$((BACKUP_COUNT - KEEP_COUNT))
  ls -t "$BACKUP_DIR"/backlog_*.db | tail -n "$EXCESS" | xargs rm -f
...
echo "📊 Total backups: $(ls -1 "$BACKUP_DIR"/backlog_*.db 2>/dev/null | wc -l)"
